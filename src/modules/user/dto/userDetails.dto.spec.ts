...
describe('userDetails', () => {
  ...
  it('should fail to validate a userDetails instance with invalid data', async () => {
    ...
    expect(errors[0].constraints['isLength']).toBe('userName must be longer than or equal to 3 characters');
  
    expect(errors[1].constraints).toHaveProperty('isLength');
    expect(errors[1].constraints['isLength']).toBe('password must be longer than or equal to 6 characters');
  });
});
